...
#  digits    Number of decimal places to print medians to
  ...
  text(x = 1, y = 1:length(sources), pos = 2, offset = 0,
       labels = formatC(meds, digits = digits, flag = '#', format = 'f'))
  # SDs
  par(mar = c(4.1, 0, 5.1, 1.1))
  plot(NULL, xlim = xlim, ylim = c(length(sds), 1), frame.plot = FALSE,
       main = 'Posterior Standard Deviation of Effects',
       xlab = '', ylab = '', yaxt = 'n')
  axis(3)
  for(j in 1:length(sources)){
    if(!is.na(sources[j])){
  ...
  points(x = meds, y = 1:length(sources), pch = '|')
  abline(v = 0)
}
...
<<data_simulation,cache=TRUE,echo=TRUE,eval=FALSE>>=
@
}

\subsection{Running GLMER}
{\footnotesize
<<glmm,cache=TRUE,echo=TRUE,eval=FALSE>>=
@
}
...
\bibitem{bda3} Gelman, A. et. al. (2013). \emph{Bayesian Data Analysis}, 3rd ed. CRC Press.
