...
We have provided code so that all details and steps of the process are available and reproducible. The simulated leaf counts are illustrated in Figure~\ref{heatmap}.
...
xtable(summary(glm.mod)$coefficient,
       caption = 'TODO',
       label = 'fixef')
xtable(as.data.frame(summary(glm.mod)$varcor)[,c(1,4,5)],
       caption = 'TODO',
       label = 'ranef')
@
...
There are many advantages to performing a Bayesian analysis. One such advantage is the ability to provide meaningful results even when there are few available observations with which to carry out inference about a given parameter.  Another is a relative robustness to unusual samples; with carefully chosen priors a Bayesian analysis can succeed here where a likelihoodist approach would utterly fail, especially in situations where the data produce an MLE value at the edge of the parameter space.

Further, a Bayesian analysis will produce a distribution with which inferences can be made about a given parameter.  A likelihoodist approach of the same data would only furnish point and variance estimates.  With the availability of posterior distributions comes not only a more complete picture regarding the parameter itself, but 2the ability to carry out posterior predictive checks and the like.
