...
Further, a Bayesian analysis will produce a distribution with which inferences can be made about a given parameter.  A likelihoodist approach of the same data would only furnish point and variance estimates.  With the availability of posterior distributions comes not only a more complete picture regarding the parameter itself, but the ability to carry out posterior predictive checks and the like.
