...
print(xtable(summary(glm.mod)$coefficient[,c('Estimate', 'Std. Error')],
             caption = 'Estimated coefficients for the constant terms.',
             label = 'fixef', digits = c(0, 3, 4)))
sumtable <- as.data.frame(summary(glm.mod)$varcor)[,c('grp', 'sdcor')]
