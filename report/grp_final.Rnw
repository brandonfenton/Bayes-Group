...
\subsection{Intro to GLMM approach}
One common approach to make inferences about split-plot experiments is to fit linear mixed models.
In this case we are analyzing simulated count data, so a generalized linear mixed model is appropriate.  
The \texttt{glm()} function from the \texttt{stats} package in the base R distribution was used to fit
this model.

As nitrogen and inoculation were pre-assigned to plots, they were treated as constants (in other words, fixed effects).  Block and variety levels can be considered as members of a larger population, and as a result will be treated as a varying, or random, effect.
<<glmm, eval=FALSE>>=
## Fit the model using glm.
glm.mod <- glmer(cbind(infected,uninfected) ~ (blk+vty+nit+ino)^3  + (1|blk),
...
In practice, we often take the natural logarithm of the likelhood. This
log-likelihood is maximized by the same \(\theta\) that maximizes the
likelihood; this is because the logarithm function is strictly increasing.
This means that if \(a\) is bigger than \(b\) then \(\log(a)\) is greater
than \(\log(b)\). For a more concrete example, see the figure on the
next page and consider the eleven points in the left panel. The diamond
without a cross has the position farthest above the horizontal axis.
The figure on the right shows the points after log-transforming their
vertical positions.


<<logex,echo=FALSE,out.height='0.3\\linewidth',fig.height=3>>=
set.seed(231486)
points <- runif(11, 0, 150)
par(mfrow = c(1, 2), mar = c(2.1, 4.1, 3.1, 2.1))
plot(points, pch = 0:10, ylim = c(0, 150),
     xlab = '', ylab = '', xaxt = 'n', main = 'Eleven Values')
plot(log(points), pch = 0:10, ylim = c(0, 5),
     xlab = '', ylab = '', xaxt = 'n',
     main = 'Natural Logarithm of the Eleven Values')
@

Notice that, after taking logarithms, the values along the vertical axis
have changed but the order of the points from highest to lowest has
stayed the same. The diamond without a cross is again the highest point
above the horizontal axis. Also, the log-tranformed points are less
spread out than the original points. This decrease in spread often happens
and makes it easier for a computer to find the maximum.

Keep in mind that the likelihood function depends on the particular data
that were observed. We could very well have collected the data \(y'\)
instead of \(y\), and then our likelihood function would be
...
\emph{estimate} has a probability distribution, but the likelihood itself
